...
const DataCleanerPage = () => {
    ...
    }, [fileId, currentPage, setPreview, setError, setIsLoading, setTotalRows, t]);
                ...
                <main className="lg:col-span-3 space-y-8">
                    <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200/50 p-8">
                        <div className="flex items-center gap-4 mb-6">
                            ...
                            <div>
                                <h2 className="text-xl font-semibold text-gray-800">{t('datacleaner.upload.title')}</h2>
                                <p className="text-gray-600">{t('datacleaner.upload.subtitle')}</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-4">
                            <label htmlFor="file-upload" className="flex items-center gap-2 px-6 py-3 bg-white border-2 rounded-xl cursor-pointer hover:border-blue-500 transition-colors">
                                ...
                                <span className="text-gray-700 font-medium">
                                    {file ? file.name : t('datacleaner.upload.selectFile')}
                                </span>
                            ...
                            <button onClick={handleUpload} disabled={!file || isLoading} className="flex items-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-xl disabled:bg-gray-400 transition-colors">
                                {isLoading ? (
                                    <>
                                        ...
                                        <span>{t('datacleaner.upload.uploading')}</span>
                                    </>
                                ) : (
                                    <>
                                        <Upload className="w-5 h-5" />
                                        <span>{t('datacleaner.upload.uploadButton')}</span>
                                    </>
                                )}
                            </button>
                        </div>
                    ...
                    {fileId && (
                        <>
                                ...
                                <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200/50 overflow-hidden">
                                    <div className="p-6 border-b border-gray-200/50 bg-gradient-to-r from-green-50 to-emerald-50">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <h2 className="text-xl font-semibold text-gray-800">{t('datacleaner.columns.title')}</h2>
                                                <p className="text-gray-600">{t('datacleaner.columns.subtitle')}</p>
                                            </div>
                                        </div>
                                    </div>
                                                    ...
                                                    <tr key={col.column} className="hover:bg-gray-50/50">
                                                        ...
                                                        <td className="px-6 py-4">
                                                            <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium ${getTypeColor(col.dtype)}`}>
                                                                {getTypeIcon(col.dtype)} {col.dtype}
                                                            </div>
                                                        </td>
                                                        <td className="px-6 py-4">
                                                            <div className={`flex items-center gap-2 ${col.nulls > 0 ? "text-amber-700" : "text-green-700"}`}>
                                                                {col.nulls > 0 ? <AlertTriangle className="w-4 h-4 text-amber-500" /> : <CheckCircle2 className="w-4 h-4 text-green-500" />}
                                                                {col.nulls}
                                                            </div>
                                                        </td>
                            ...
                            {columns.length > 0 && columns.some(col => col.nulls > 0) && (
                                <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl p-6">
                                    <div className="flex items-center gap-3 mb-6">
                                        ...
                                        <div>
                                            <h2 className="text-xl font-semibold text-gray-800">{t('datacleaner.imputation.title')}</h2>
                                            <p className="text-gray-600">{t('datacleaner.imputation.subtitle')}</p>
                                        </div>
                                    </div>
                                            ...
                                            {columns.filter((col) => col.nulls > 0).map((col) => (
                                                <label key={col.column} className="relative">
                                                    <input
                                                        type="checkbox"
                                                        value={col.column}
                                                        checked={selectedMissingCols.includes(col.column)}
                                                        onChange={(e) => {
                                                            const val = e.target.value;
                                                            setSelectedMissingCols((prev) =>
                                                    ...
                                                    <div className={`p-4 rounded-xl border-2 cursor-pointer transition-all ${selectedMissingCols.includes(col.column) ? "border-purple-500 bg-purple-50" : "border-gray-200 bg-white hover:border-gray-300"}`}>
                                                        ...
                                                        <div className="text-sm text-amber-600">{t('datacleaner.imputation.missing', { count: col.nulls })}</div>
                                                    </div>
                                                </label>
                                            ))}
                                        </div>
                                    ...
                                    <button
                                        ...
                                        disabled={isImputing || selectedMissingCols.length === 0}
                                        className="mt-6 flex items-center gap-2 px-6 py-3 bg-purple-600 text-white rounded-xl disabled:bg-gray-400 transition-colors"
                                    >
                                        {isImputing ? (
                                            <>
                                                <Loader className="w-5 h-5 animate-spin" />
                                                <span>{t('datacleaner.imputation.filling')}</span>
                                            </>
                                        ) : (
                                            t('datacleaner.imputation.fillButton')
                                        )}
                                    </button>
                                </div>
                            )}
                            ...
                            {columns.length > 0 && (
                                <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl p-6">
                                    <div className="flex items-center gap-3 mb-6">
                                        ...
                                        <div>
                                            <h2 className="text-xl font-semibold text-gray-800">{t('datacleaner.outliers.title')}</h2>
                                            <p className="text-gray-600">{t('datacleaner.outliers.subtitle')}</p>
                                        </div>
                                    </div>
                                            ...
                                            {columns.filter((col) => col.dtype === "int64" || col.dtype === "float64").map((col) => (
                                                <label key={col.column} className="relative">
                                                    <input
                                                        type="checkbox"
                                                        value={col.column}
                                                        checked={selectedOutlierCols.includes(col.column)}
                                                        onChange={(e) => {
                                                            const val = e.target.value;
                                                            setSelectedOutlierCols((prev) =>
                                                                prev.includes(val) ? prev.filter((c) => c !== val) : [...prev, val]
                                                            );
                                                        }}
                                                        className="sr-only"
                                                    ...
                                                    <div className={`p-4 rounded-xl border-2 cursor-pointer transition-all ${selectedOutlierCols.includes(col.column) ? "border-red-500 bg-red-50" : "border-gray-200 bg-white hover:border-gray-300"}`}>
                                                        <div className="font-medium text-gray-900">{col.column}</div>
                                                        <div className="text-sm text-gray-500">{t('datacleaner.outliers.unique', { count: col.unique })}</div>
                                                    </div>
                                                </label>
                                            ))}
                                        </div>
                                    ...
                                    <button
                                        ...
                                        disabled={isAnalyzing || selectedOutlierCols.length === 0}
                                        className="mt-6 flex items-center gap-2 px-6 py-3 bg-red-600 text-white rounded-xl disabled:bg-gray-400 transition-colors"
                                    >
                                        {isAnalyzing ? (
                                            <>
                                                <Loader className="w-5 h-5 animate-spin" />
                                                <span>{t('datacleaner.outliers.analyzing')}</span>
                                            </>
                                        ) : (
                                            t('datacleaner.outliers.detectButton')
                                        )}
                                    </button>
                                </div>
                            )}
                                    ...
                                    <div className="p-6 border-b border-red-200/50 bg-gradient-to-r from-red-50 to-pink-50">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-red-600">{t('datacleaner.outliers.foundSubtitle')}</p>
                                            </div>
                                        </div>
                                    </div>
                                    {outliers.length > 0 ? (
                                                ...
                                                <thead className="bg-red-50/80">
                                                    <tr>
                                                        {Object.keys(outliers[0]).map((key) => (
                                                            <th key={key} className="px-6 py-4 text-left text-sm font-semibold text-red-700">{key}</th>
                                                        ))}
                                                    </tr>
                                                </thead>
                                                    ...
                                                    {outliers.map((row, idx) => (
                                                        <tr key={idx}>
                                                            {Object.values(row).map((val: any, i) => (
                                                                <td key={i} className="px-6 py-4 text-red-900">{String(val)}</td>
                                                            ))}
                                                        </tr>
                                                    ))}
                                                </tbody>
                            ...
                            {columns.length > 0 && columns.some(col => col.dtype === 'object') && (
                                <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl p-6">
                                    <div className="flex items-center gap-3 mb-6">
                                        <div className="p-2 bg-gradient-to-r from-cyan-500 to-sky-500 rounded-xl">
                                            <Zap className="w-6 h-6 text-white" />
                                        </div>
                                        <div>
                                            <h2 className="text-xl font-semibold text-gray-800">{t('datacleaner.encoding.title')}</h2>
                                            <p className="text-gray-600">{t('datacleaner.encoding.subtitle')}</p>
                                        </div>
                                    </div>
                                            ...
                                            {columns.filter((col) => col.dtype === 'object').map((col) => (
                                                <label key={col.column} className="relative">
                                                    <input
                                                        type="checkbox"
                                                        value={col.column}
                                                        checked={selectedEncodingCols.includes(col.column)}
                                                        onChange={(e) => {
                                                            const val = e.target.value;
                                                            setSelectedEncodingCols((prev) =>
                                                                prev.includes(val) ? prev.filter((c) => c !== val) : [...prev, val]
                                                            );
                                                        }}
                                                        className="sr-only"
                                    ...
                                    <button
                                        ...
                                        disabled={isEncoding || selectedEncodingCols.length === 0}
                                        className="mt-6 flex items-center gap-2 px-6 py-3 bg-cyan-600 text-white rounded-xl disabled:bg-gray-400 transition-colors"
                                    >
                                        {isEncoding ? (
                                            <>
                                                <Loader className="w-5 h-5 animate-spin" />
                                                <span>{t('datacleaner.encoding.encoding')}</span>
                                            </>
                                        ) : (
                                            t('datacleaner.encoding.encodeButton')
                                        )}
                                    </button>
                                </div>
                            )}
                            ...
                            {preview.length > 0 && (
                                <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200/50 overflow-hidden">
                                    <div className="p-6 border-b border-gray-200/50">
                                        <h2 className="text-xl font-semibold text-gray-800">{t('datacleaner.preview.title')}</h2>
                                    </div>
                                            ...
                                            <thead className="bg-gray-50/80">
                                                <tr>
                                                    {Object.keys(preview[0]).map((key) => (
                                                        <th key={key} className="px-6 py-4 text-left text-sm font-semibold text-gray-700">{key}</th>
                                                    ))}
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-gray-200/50">
                                                {preview.map((row, idx) => (
                                                    <tr key={idx} className="hover:bg-gray-50/50">
                                                        {Object.values(row).map((val: any, i) => (
                                                            <td key={i} className="px-6 py-4 text-gray-800">{String(val)}</td>
                                                        ))}
                                                    </tr>
                                                ))}
                                            </tbody>
                                    ...
                                    {totalRows > 0 && (
                                        <div className="flex items-center justify-between p-4 border-t border-gray-200/50">
                                            <span className="text-sm text-gray-700">
                                                {t('datacleaner.preview.totalRows', { count: totalRows })}
                                            </span>
