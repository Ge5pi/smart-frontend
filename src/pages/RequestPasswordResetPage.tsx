  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.response && err.response.data && err.response.data.detail) {
        ...
        setError(t('passwordReset.errorDefault'));
      }
      console.error(err);
    } finally {
      setIsLoading(false);
    ...
    <div className="max-w-md mx-auto px-6 py-12">
      <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200/50 p-8">
        <div className="flex flex-col items-center mb-6">
          ...
          <h2 className="text-2xl font-bold text-gray-800 mt-4">{t('passwordReset.title')}</h2>
          <p className="text-gray-600 mt-1">{t('passwordReset.subtitle')}</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-2 px-6 py-4 bg-blue-600 text-white rounded-xl font-semibold"
          >
            {isLoading ? <Loader className="w-5 h-5 animate-spin" /> : t('passwordReset.submit')}
          </button>
        </form>
      </div>
    </div>
