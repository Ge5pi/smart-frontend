...
const SubscriptionPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      setError(t('subscription.errorNameRequired'));
      return;
    }
    ...
    try {
      ...
      const errorMessage = err.response?.data?.detail ?? t('subscription.errorDefault');
  ...
  if (success) {
    return (
      <div className="max-w-md mx-auto mt-10 text-center p-8 bg-white rounded-lg shadow-lg">
        ...
        <h1 className="text-2xl font-bold text-gray-800 mb-2">{t('subscription.successTitle')}</h1>
        <p className="text-gray-600">
          {t('subscription.successMessage1')} <span className="font-semibold">{user.email}</span>. {t('subscription.successMessage2')}
        </p>
        <button
          onClick={() => navigate('/')}
      ...
      <div className="max-w-md w-full space-y-8 p-10 bg-white rounded-xl shadow-lg">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            >
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : t('subscription.submit')}
            </button>
          </div>
        </form>
      </div>
