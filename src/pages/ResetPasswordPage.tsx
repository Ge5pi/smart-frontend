...
const ResetPasswordPage = () => {
  ...
  useEffect(() => {
    if (!token) {
      setError(t('resetPassword.errorNoLink'));
    }
  }, [token, t]);
    ...
    <div className="max-w-md mx-auto px-6 py-12">
      <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200/50 p-8">
        <div className="flex flex-col items-center mb-6">
          ...
          <p className="text-gray-600 mt-1">{t('resetPassword.subtitle')}</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {isLoading ? <Loader className="w-5 h-5 animate-spin" /> : t('resetPassword.submit')}
          </button>
        </form>
      </div>
    </div>
